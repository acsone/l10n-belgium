...
def generate_xml(author, year, filename, data_file, calc_file):
    ...
    _create_translation_file("l10n_be_mis_reports.pot", json_data_dict)
    # FR
    _create_translation_file("fr.po", json_data_dict, "fr")
    # NL
    _create_translation_file("nl.po", json_data_dict, "nl")
    # DE
    _create_translation_file("de.po", json_data_dict, "de")
...
def _generate_balance_sheet(author, year, filename, data_dict, calc_dict):
    ...
    bs_doc_name = balance_sheet["section"]["labels"]["en"] + f" [{filename}]"
    ...
    _logger.info(f"file mis_report_bs_{filename}.xml sucessfully created.")
...
def _generate_profit_loss(author, year, filename, data_dict, calc_dict):
    ...
    pl_doc_name = profit_and_loss["section"]["labels"]["en"] + f" [{filename}]"
    parsed_filename = filename.replace("-", "_")
    ...
    _logger.info(f"file mis_report_pl_{filename}.xml sucessfully created.")


def _create_translation_file(filename, data_dict, lang=""):
    """
    This method creates the translation file for a language
    :param filename: The name of the file where the translations will be written
    :param data_dict: data dictionary containing data from the NBB taxonomy
    :param lang: The ISO 639-1 code of the language
    """
    if not os.path.exists("../i18n"):
        os.makedirs("../i18n")
    with open(f"../i18n/{filename}", "w") as pot_file:
        if len(lang) == 0:
            pot_file.write(
...
"""
            ...
            with open(f"../i18n/{filename}", "w") as file:
                file.write(
                    """# Translation of Odoo Server.
# This file contains the translation of the following modules:
# * l10n_be_mis_reports
...
"""
    ...
    _write_translation(filename, balance_sheet, lang)
    _write_translation(filename, profit_and_loss, lang)


def _write_translation(filename, rubcodes_dict, lang=""):
    """
    This method writes a rubcode name and its translation in the selected language
    :param filename: The name of the file where the translations will be written
    :param rubcodes_dict: dictionary containing the data regarding the rubric codes
    :param lang: The ISO 639-1 code of the language
    """
    rub_sections = rubcodes_dict.get("section").get("sectionsOrTables")
    for rub_section in rub_sections:
        tables = (
            rub_section.get("section").get("sectionsOrTables")
            if rub_section.get("section")
            else [rub_section]
        )
        for table in tables:
            rows = table.get("table").get("rows")
            for row in rows:
                if len(row.get("cols")) < 2:
                    continue
                if row.get("cols")[-2].get("fp"):
                    rubcode = row.get("cols")[-2].get("fp").get("rubCode")
                    sanitized_rubcode = _get_sanitized_acc_name(rubcode)
                    with open(f"../i18n/{filename}", "a") as file:
                        file.write("#. module: l10n_be_mis_reports\n")
                        file.write(
                            f"#: model:mis.report.kpi,description:"
                            f"l10n_be_mis_reports.new_mis_report_bs_{sanitized_rubcode}\n"
                        )
                        file.write(
                            f"msgid \"{_get_account_label(row.get('cols'))} [{rubcode}]\"\n"
                        )
                        account_label = _get_account_label(row.get("cols"), lang)
                        if account_label:
                            file.write(f'msgstr "{account_label} [{rubcode}]"\n\n')
                        else:
                            file.write('msgstr ""\n\n')
